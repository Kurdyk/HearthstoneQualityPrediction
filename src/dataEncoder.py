...
class DataEncoder:
	def __init__(self, class_encoder: ClassEncoder, minion_type_encoder: MinionTypeEncoder,
				 rarity_encoder: RarityEncoder, spell_school_encoder: SpellSchoolsEncoder,
				 text_encoder: TextEncoder, data_frame: pd.DataFrame):
		...
		self.df = data_frame.drop(columns="card_type")

	def encode(self, n_dim_text: int):

		class_encoded = self.class_encoder.encode_class_col_one_hot(self.df)
...
if __name__ == "__main__":
	...
	data_encoder = DataEncoder(ClassEncoder(), MinionTypeEncoder(), RarityEncoder(),
							   SpellSchoolsEncoder(), TextEncoder(), df)

	data_encoder.encode(20).to_csv("encoded_data.csv")
